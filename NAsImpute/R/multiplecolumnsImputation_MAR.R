multiplecolumnImputation_MAR = function(data, k, ntree, rows, columns = c(1:dim(data)[2]), mar_columns, iterations, histogram = FALSE) {
...
for ( i in 1:iterations ) {
  tmp_MAE_random[i] = mean(MAR_multiple_random[[i]][which(!is.na(MAR_multiple_random[[i]][,1])),1])
  tmp_RMSE_random[i] = mean(MAR_multiple_random[[i]][which(!is.na(MAR_multiple_random[[i]][,2])),2])
  tmp_Euclidean_random[i] = mean(MAR_multiple_random[[i]][which(!is.na(MAR_multiple_random[[i]][,3])),3]) }

Histograms = list()
multiplecolumn = list()
multiplecolumn_random = list()

MAR_multiple_mean = list()
for( i in 1:iterations ) {
  C = c()
  tmp = data.frame(MAE = c(rep(x = NA, times = max(columns))), RMSE = c(rep(x = NA, times = max(columns))), Euclidean = c(rep(x = NA, times = max(columns))))
  rownames(tmp) = colnames(data)[1:max(columns)]
...
for( h in columns ) {
  for( i in 1:iterations ) {
    tmp[i,1] = mean(MAR_multiple_mean[[i]][which(!is.na(MAR_multiple_mean[[i]][,1])),1])
    tmp[i,2] = mean(MAR_multiple_mean[[i]][which(!is.na(MAR_multiple_mean[[i]][,2])),2])
    tmp[i,3] = mean(MAR_multiple_mean[[i]][which(!is.na(MAR_multiple_mean[[i]][,3])),3]) }
...
for ( i in 1:iterations ) {
  tmp_MAE_mean[i] = mean(MAR_multiple_mean[[i]][which(!is.na(MAR_multiple_mean[[i]][,1])),1])
  tmp_RMSE_mean[i] = mean(MAR_multiple_mean[[i]][which(!is.na(MAR_multiple_mean[[i]][,2])),2])
  tmp_Euclidean_mean[i] = mean(MAR_multiple_mean[[i]][which(!is.na(MAR_multiple_mean[[i]][,3])),3])
...
for( i in 1:iterations ) {
  ...
  MAR_multiple_KNN[[i]] = tmp }

tmp = data.frame(MAE = c(rep(x = NA, times = iterations)), RMSE = c(rep(x = NA, times =iterations)), Euclidean = c(rep(x = NA, times = iterations)))
multiplecolumn$KNN = c(MAE = NA, RMSE = NA, Euclidean = NA)
for( h in columns ) {
  for( i in 1:iterations ) {
    tmp[i,1] = mean(MAR_multiple_KNN[[i]][which(!is.na(MAR_multiple_KNN[[i]][,1])),1])
    tmp[i,2] = mean(MAR_multiple_KNN[[i]][which(!is.na(MAR_multiple_KNN[[i]][,2])),2])
    tmp[i,3] = mean(MAR_multiple_KNN[[i]][which(!is.na(MAR_multiple_KNN[[i]][,3])),3]) }
...
for ( i in 1:iterations ) {
  tmp_MAE_KNN[i] = mean(MAR_multiple_KNN[[i]][which(!is.na(MAR_multiple_KNN[[i]][,1])),1])
  tmp_RMSE_KNN[i] = mean(MAR_multiple_KNN[[i]][which(!is.na(MAR_multiple_KNN[[i]][,2])),2])
  tmp_Euclidean_KNN[i] = mean(MAR_multiple_KNN[[i]][which(!is.na(MAR_multiple_KNN[[i]][,3])),3])
...
for( i in 1:iterations ) {
  ...
  MAR_multiple_amelia[[i]] = tmp }

tmp = data.frame(MAE = c(rep(x = NA, times = iterations)), RMSE = c(rep(x = NA, times =iterations)), Euclidean = c(rep(x = NA, times = iterations)))
multiplecolumn$amelia = c(MAE = NA, RMSE = NA, Euclidean = NA)
for( h in columns ) {
  for( i in 1:iterations ) {
    tmp[i,1] = mean(MAR_multiple_amelia[[i]][which(!is.na(MAR_multiple_amelia[[i]][,1])),1])
    tmp[i,2] = mean(MAR_multiple_amelia[[i]][which(!is.na(MAR_multiple_amelia[[i]][,2])),2])
    tmp[i,3] = mean(MAR_multiple_amelia[[i]][which(!is.na(MAR_multiple_amelia[[i]][,3])),3]) }
...
for ( i in 1:iterations ) {
  tmp_MAE_amelia[i] = mean(MAR_multiple_amelia[[i]][which(!is.na(MAR_multiple_amelia[[i]][,1])),1])
  tmp_RMSE_amelia[i] = mean(MAR_multiple_amelia[[i]][which(!is.na(MAR_multiple_amelia[[i]][,2])),2])
  tmp_Euclidean_amelia[i] = mean(MAR_multiple_amelia[[i]][which(!is.na(MAR_multiple_amelia[[i]][,3])),3])
...
for( i in 1:iterations ) {
  ...
  MAR_multiple_mice[[i]] = tmp }

tmp = data.frame(MAE = c(rep(x = NA, times = iterations)), RMSE = c(rep(x = NA, times =iterations)), Euclidean = c(rep(x = NA, times = iterations)))
multiplecolumn$mice = c(MAE = NA, RMSE = NA, Euclidean = NA)
for( h in columns ) {
  for( i in 1:iterations ) {
    tmp[i,1] = mean(MAR_multiple_mice[[i]][which(!is.na(MAR_multiple_mice[[i]][,1])),1])
    tmp[i,2] = mean(MAR_multiple_mice[[i]][which(!is.na(MAR_multiple_mice[[i]][,2])),2])
    tmp[i,3] = mean(MAR_multiple_mice[[i]][which(!is.na(MAR_multiple_mice[[i]][,3])),3]) }
...
for ( i in 1:iterations ) {
  tmp_MAE_mice[i] = mean(MAR_multiple_mice[[i]][which(!is.na(MAR_multiple_mice[[i]][,1])),1])
  tmp_RMSE_mice[i] = mean(MAR_multiple_mice[[i]][which(!is.na(MAR_multiple_mice[[i]][,2])),2])
  tmp_Euclidean_mice[i] = mean(MAR_multiple_mice[[i]][which(!is.na(MAR_multiple_mice[[i]][,3])),3])
...
for( i in 1:iterations ) {
  ...
  MAR_multiple_missForest[[i]] = tmp }

tmp = data.frame(MAE = c(rep(x = NA, times = iterations)), RMSE = c(rep(x = NA, times =iterations)), Euclidean = c(rep(x = NA, times = iterations)))
multiplecolumn$missForest = c(MAE = NA, RMSE = NA, Euclidean = NA)
for( h in columns ) {
  for( i in 1:iterations ) {
    tmp[i,1] = mean(MAR_multiple_missForest[[i]][which(!is.na(MAR_multiple_missForest[[i]][,1])),1])
    tmp[i,2] = mean(MAR_multiple_missForest[[i]][which(!is.na(MAR_multiple_missForest[[i]][,2])),2])
    tmp[i,3] = mean(MAR_multiple_missForest[[i]][which(!is.na(MAR_multiple_missForest[[i]][,3])),3]) }
...
for ( i in 1:iterations ) {
  tmp_MAE_missForest[i] = mean(MAR_multiple_missForest[[i]][which(!is.na(MAR_multiple_missForest[[i]][,1])),1])
  tmp_RMSE_missForest[i] = mean(MAR_multiple_missForest[[i]][which(!is.na(MAR_multiple_missForest[[i]][,2])),2])
  tmp_Euclidean_missForest[i] = mean(MAR_multiple_missForest[[i]][which(!is.na(MAR_multiple_missForest[[i]][,3])),3])
...
for( i in 1:iterations ) {
  ...
  MAR_multiple_MI[[i]] = tmp }

tmp = data.frame(MAE = c(rep(x = NA, times = iterations)), RMSE = c(rep(x = NA, times =iterations)), Euclidean = c(rep(x = NA, times = iterations)))
multiplecolumn$MI = c(MAE = NA, RMSE = NA, Euclidean = NA)
for( h in columns ) {
  for( i in 1:iterations ) {
    tmp[i,1] = mean(MAR_multiple_MI[[i]][which(!is.na(MAR_multiple_MI[[i]][,1])),1])
    tmp[i,2] = mean(MAR_multiple_MI[[i]][which(!is.na(MAR_multiple_MI[[i]][,2])),2])
    tmp[i,3] = mean(MAR_multiple_MI[[i]][which(!is.na(MAR_multiple_MI[[i]][,3])),3]) }
...
for ( i in 1:iterations ) {
  tmp_MAE_MI[i] = mean(MAR_multiple_MI[[i]][which(!is.na(MAR_multiple_MI[[i]][,1])),1])
  tmp_RMSE_MI[i] = mean(MAR_multiple_MI[[i]][which(!is.na(MAR_multiple_MI[[i]][,2])),2])
  tmp_Euclidean_MI[i] = mean(MAR_multiple_MI[[i]][which(!is.na(MAR_multiple_MI[[i]][,3])),3])
